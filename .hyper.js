...
module.exports = {
    config: {
        ...
        // if you're using a Linux setup which show native menus, set to false default:
        // `true` on Linux, `true` on Windows, ignored on macOS
        showHamburgerMenu: '',

        // set to `false` (without backticks and without quotes) if you want to hide the
        // minimize, maximize and close buttons additionally, set to `'left'` if you
        // want them on the left, like in Ubuntu default: `true` (without backticks and
        ...
        // if `true` (without backticks and without quotes), hyper will be set as the
        // default protocol client for SSH
        defaultSSHApp: true,

        // if `true` (without backticks and without quotes), on right click selected
        // text will be copied or pasted if no selection is present (`true` by default
        // on Windows and disables the context menu feature) quickEdit: true, URL to
        // custom bell bellSoundURL: 'http://example.com/bell.mp3', for advanced config
        // flags please refer to https://hyper.is/#cfg
    },
